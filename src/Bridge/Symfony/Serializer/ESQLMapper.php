...
final class ESQLMapper implements ESQLMapperInterface
{
    ...
    public function __construct(DenormalizerInterface $normalizer, ESQLInterface $esql)
    {
    ...
    public function map(array $data, string $resourceClass)
    {
        ...
        if (isset($memory[$resourceClass])) {
            ...
            foreach ($memory as $class => $value) {
                $normalized[$relationFieldName($class)] = $this->normalizer->denormalize($value, $class, null, [ObjectNormalizer::DISABLE_TYPE_ENFORCEMENT => true]);
                unset($memory[$class]);
            }
